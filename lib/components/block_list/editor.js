...
class Editor extends BlockList {
  ...
  findCodeParent(block, closest){
    if(closest === this.root || closest.connected === "not yet" ){
      return this.resetChildren();
    }

    if(closest.connected) CodeParent.addChild(block, closest);
    }
  }
// if closestBlock === root
// addRoot as Parent
// if
// Check if the closest parent is root. If it is then add root as the parent.

// Check if the closestBlock is a active Parent, if so, make it your parent and put it on the front.


// If its not, check if its the last block of it's parent. If so, add root as a parent.


// Lastly, if its not the parent, then insert the code, in it's correct location inside the parent.
