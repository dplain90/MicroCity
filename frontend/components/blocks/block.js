class Block extends createjs.Container {
  ...
  setup() {
    this.addChild(this.chipImg, this.label);
  }

  createLabel(name, color){
    this.label = new createjs.Text(name.toUpperCase(), "6.5px Audiowide, cursive", color);
  }
  ...
  remove(){
    this.prev.next = this.next if this.prev !== null;
    this.next.prev = this.prev if this.next !== null;
...
class Block {
  ...
  createLabel(color){
    let label = new createjs.Text(name.toUpperCase(), "6.5px Audiowide, cursive", color);
